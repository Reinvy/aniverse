...
test.describe("Navigation", () => {
  ...
  test("should navigate to blog detail page from blog listing", async ({
    page,
  }) => {
  ...
  test("should navigate to character detail page from character listing", async ({
    page,
  }) => {
    await page.goto("/characters");
    // Character cards link to /characters/[id] (CSR list — wait for render)
    ...
    await expect(page.locator("body")).toBeVisible();
    // Detail page should render the profile (h1 name + design-system markers)
    await expect(page.locator("h1").first()).toBeVisible({ timeout: 10000 });
    await expect(page.getByText("CHARACTER // PROFILE").first()).toBeVisible({
      timeout: 10000,
    });
    // Back link returns to the directory
    await page
      .getByRole("link", { name: /back to characters/i })
      .first()
      .click();
    await expect(page).toHaveURL(/\/characters$/);
  });

  test("unknown character id should render graceful not-found (no crash)", async ({
    page,
  }) => {
    await page.goto("/characters/this-id-does-not-exist-xyz");
    await expect(
      page.getByText(/character.*not found|doesn't exist/i).first(),
  ...
  test("landing page quick links should navigate to real pages", async ({
    ...
    const aboutBtn = page
      .locator('nav button:has-text("About"), nav a:has-text("About")')
      .first();
    await aboutBtn.click();
    await page.waitForTimeout(1000);

    const quickLinks = page.locator(
      'a[href="/blog"], a[href="/characters"], a[href="/challenges"], a[href="/dashboard/gallery"]',
    );
  ...
  test("landing page footer strip links should navigate to real pages", async ({
    page,
  }) => {
    ...
    const footerLinks = page.locator(
      'a[href="/blog"], a[href="/challenges"], a[href="/characters"], a[href="/register"]',
    );
